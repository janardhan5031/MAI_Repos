import { LoggerService, Inject, Logger, Injectable } from '@nestjs/common';
...
export class LoggingService extends Logger {
  ...
  log(message: string) {
    ...
    this.fs.appendFile(this.LOG_PATH.replace('%DATE%', moment().format('YYYY-MM-DD').toString()), JSON.stringify(logMessage) + "\n", (err) => {
      if(err) throw err;
  ...
  error(message: string, trace: any) {
    ...
    this.fs.appendFile(this.LOG_PATH, JSON.stringify(logMessage) + "\n", (err) => {
      if(err) throw err;
